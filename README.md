...
Clone the repo and run **one** command. No system Python needed, no
admin rights, no PATH surgery — everything (a standalone Python 3.10,
the venv, dependencies, the voice model) gets installed *inside the
project folder*.
...
```powershell
# Windows — or just double-click run.bat in Explorer
git clone https://github.com/nischaljs/nepanglish-tts.git
cd nepanglish-tts
.\run.ps1
```

First run sets everything up (≈5 minutes, mostly downloads); after that
it just launches the type-and-hear REPL. Re-runs are cheap. Delete the
project folder to uninstall everything — nothing leaks system-wide.

Useful flags:

| Command                              | What it does                                          |
|--------------------------------------|-------------------------------------------------------|
| `bash run.sh`                        | Set up if needed, then launch the REPL                |
| `bash run.sh setup`                  | Set up only — don't launch anything                   |
| `bash run.sh -- python -c "..."`     | Run any command inside the project venv               |

(Windows: same flags, `.\run.ps1` instead of `bash run.sh`.)
...
```bash
bash run.sh                # Linux / macOS / Pi
.\run.ps1                  # Windows  (or double-click run.bat)
```

That's it — `run` handles setup on first invocation and drops you
straight into the REPL on every subsequent run. If you'd rather drive
the venv yourself:
